...
const BATCH_SIZE = 5; // Smaller batch size for this one-time update
const DELAY_BETWEEN_BATCHES = 3000; // 3 seconds between batches
const LOG_FILE = path.join(__dirname, 'description-update-log.json');
...
async function notifyDescriptionUpdates() {
  try {
    if (!fs.existsSync(inputFile)) {
      console.error('No recently updated jobs file found. Run fix-description-format.js first.');
      process.exit(1);
    }
    ...
    if (updatedFiles.length === 0) {
      console.log('No files to process. Run fix-description-format.js first.');
      process.exit(0);
    }
    ...
    console.log(`Found ${urls.length} URLs to process from description format updates`);
    ...
    console.log('\nDescription Update Indexing Summary:');
    console.log('----------------------------------');
    ...
    // Save final results
    const summaryFile = path.join(__dirname, 'description-update-summary.json');
